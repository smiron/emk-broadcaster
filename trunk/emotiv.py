...
class Emotiv(object):
  ...
  def setupWin(self, headsetId):
    filter = hid.HidDeviceFilter(vendor_id=0x21A1, product_name='00000000000e')
    ...
    print 'before'
    print '-------------'
    print self.device.serial_number
    print '-------------'
    ...
    def handle(data):      
      assert data[0] == 0
      self.gotData(''.join(map(chr, data[1:])),sock)
    print 'after defined' 
